...
const TrainingMode: React.FC<TrainingModeProps> = ({ onOpenKey }) => {
  ...
  const [selectedAction, setSelectedAction] = useState<'FOLD' | 'HOLD' | 'RAISE' | 'ALL-IN' | null>(null);
  ...
  const generateScenario = () => {
    ...
    const opponentProfile = (['standard', 'aggressive', 'passive', 'bluffer', 'loose'] as OpponentProfile[])[Math.floor(Math.random() * 5)];
      ...
      {!scenario ? (
        <div className="bg-emerald-900/40 border border-white/5 rounded-[3rem] p-12 text-center space-y-8 max-w-2xl mx-auto mt-20">
          ...
          <div className="space-y-4">
            ...
            <p className="text-emerald-300/60 text-sm max-w-xs mx-auto font-medium">Ejercicios de toma de decisiones GTO con crítica instantánea.</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
          <div className="bg-emerald-900/40 border border-white/5 rounded-[2.5rem] p-8 space-y-8">
            <div className="flex flex-wrap gap-2">
              ...
              <span className="px-4 py-1 bg-white/10 text-emerald-300 border border-white/10 rounded-full text-[9px] font-black uppercase tracking-widest">
                RIVAL: {scenario.opponentProfile.toUpperCase()}
              </span>
            </div>
            ...
            <div className="pt-8 border-t border-white/5 space-y-6">
              <h4 className="text-center font-black text-sm uppercase tracking-widest">¿Qué harías en esta situación?</h4>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { id: 'FOLD', label: 'Tirarse', icon: '🏳️', color: 'hover:bg-rose-500/20 border-rose-500/30' },
                  { id: 'HOLD', label: 'Hold', icon: '🤝', color: 'hover:bg-blue-500/20 border-blue-500/30' },
                  { id: 'RAISE', label: 'Raise', icon: '↗️', color: 'hover:bg-amber-500/20 border-amber-500/30' },
                  { id: 'ALL-IN', label: 'All-in', icon: '💣', color: 'hover:bg-purple-500/20 border-purple-500/30' },
                ].map(action => (
                  <button
                    key={action.id}
                    disabled={!!analysis || isAnalyzing}
                    onClick={() => setSelectedAction(action.id as any)}
                    className={`p-4 rounded-2xl border flex flex-col items-center gap-2 transition-all ${action.color} ${selectedAction === action.id ? 'bg-white/20 border-white/40 scale-[1.02] shadow-xl' : 'bg-white/5 border-white/5 opacity-60'}`}
                  >
                    <span className="text-2xl">{action.icon}</span>
                    <span className="text-[10px] font-black uppercase text-white">{action.label}</span>
                  </button>
                ))}
              </div>
              {!analysis && (
                <button
                  ...
                  className={`w-full py-5 font-black rounded-2xl shadow-xl transition-all flex items-center justify-center gap-3 uppercase text-xs ${selectedAction ? 'bg-emerald-500 text-white hover:scale-[1.02]' : 'bg-white/5 text-white/20'}`}
                >
                  {isAnalyzing ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'COMPROBAR DECISIÓN'}
                </button>
              )}
            </div>
          </div>
            ...
            {analysis ? (
              <div className="h-full bg-slate-900 rounded-[2.5rem] p-8 border border-white/5 flex flex-col items-center justify-center space-y-8 animate-in zoom-in-95 duration-500">
                <div className="text-center space-y-2">
                  <span className="text-[10px] font-black text-white/40 uppercase tracking-widest">Equity Estimada</span>
